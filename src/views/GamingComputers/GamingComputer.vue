<template>
    ...
    <div style="display: flex; flex-direction: column; align-items: center; margin-top: 20px;">
      ...
      <div style="margin-top: -50px; width: 100%; max-width: 300px;">
        <select v-model="selectedCategory" id="categoryFilter" class="custom-select">
          ...
          <option value="samsung">Laptops</option>
          <option value="huawei">Desktops</option>
        </select>
      </div>
    </div>
  <p></p>
  <p></p>
    <div class="product-cards">
      <router-link v-for="product in filteredProducts" :key="product._id" :to="`/gamingcomputers/${product._id}`" class="product-card-link">
  ...
  try {
    ...
    products.value = data;
    const desiredCategories = ['gaminglaptop', 'gamingdesktop'];
